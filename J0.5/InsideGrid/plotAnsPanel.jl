...
Ttp = vstack( plot( 
      layer(x=xdis, y=Ta[:,1], #=ymin=sdmin[:,1], ymax=sdmax[:,1], Geom.point, Geom.errorbar=# Geom.bar, Theme(default_color=colorant"red", bar_spacing=6mm))
...
,  plot(
      layer(x=xdis, y=Ta[:,3], #=ymin=sdmin[:,2], ymax=sdmax[:,2], Geom.point, Geom.errorbar,=# Geom.bar, Theme(default_color=colorant"red", bar_spacing=6mm))
      ,layer(x=xdis, y=Ttrue[:,2], Geom.bar)
      , Scale.x_discrete, Guide.xlabel(""), Guide.ylabel("Shift")
      , Guide.manual_color_key("Legend", ["True Value", "Found Value"], ["blue", "red"])
      ,Theme(minor_label_font_size = 22pt), Guide.yticks(ticks = [-10, 0, 10]), Guide.xticks(ticks = [1, 4, 8, 12, 16]))
, plot(
      layer(x=xdis, y=Ta[:,2], #=ymin=sdmin[:,3], ymax=sdmax[:,3], Geom.point, Geom.errorbar,=# Geom.bar, Theme(default_color=colorant"red", bar_spacing=6mm)) 
...
#/////////// Location Plot //////////////////
SoPos = readcsv("./InputInGrid/SourcePosition.csv");
micPos = readcsv("./InputInGrid/MicPosition.csv");
